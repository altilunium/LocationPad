# Altilunium LocationPad

https://altilunium.github.io/LocationPad/

<img width="1365" height="659" alt="image" src="https://github.com/user-attachments/assets/99fc804b-7934-4528-bf9d-bcada9a62a20" />


A lightweight web app for marking, labeling, and revisiting meaningful places on an OpenStreetMap-based map. Designed for quick place logging, personal mapping, and spatial note-taking without accounts.

## How to use:

To add a new place, either paste the coordinates or right-click on the map (tap and hold on mobile).

<img width="810" height="426" alt="image" src="https://github.com/user-attachments/assets/7e585bae-62c0-4f7c-9f7b-7843b306a22e" />



## Developer blog
* [v26.9.5](https://mastodon.social/@rphyrin/117217167068179862) : Add Valhalla routing UI and measurement features
* [v26.5.22](https://www.openstreetmap.org/user/rphyrin/diary/408718) :  Support for long-form place descriptions.
* [v26.5.20](https://www.openstreetmap.org/user/rphyrin/diary/408702) : Updated file input to accept .geojson format and modified export/import functions to handle GeoJSON data structure.
* [v26.4.6](https://www.openstreetmap.org/user/rphyrin/diary/408474) : This update introduces a more flexible workspace system by allowing users to create, edit, delete, and switch between multiple “canvas” environments. Each canvas now operates with its own independent database, making it easier to separate and manage different datasets or projects. The canvas manager behavior has also been refined. When the Ctrl + . shortcut is triggered, the canvas manager will now be properly hidden to ensure a cleaner interface and avoid visual clutter during use. Interaction with markers has been improved as well. Clicking on a marker will now display its precise coordinates, providing clearer spatial information directly within the interface. Finally, data portability has been expanded. Each canvas now includes an “Export to JSON” option, allowing users to easily back up or share their data. In addition, a new “Import from JSON” feature enables users to load a JSON file into a newly created canvas, simplifying data transfer and reuse across environments.
* v26.1.25 : Press Ctrl+. to toggle the visibility of UI elements.
* v26.1.8 : "[A trip to the Ragunan Zoo](https://www.openstreetmap.org/user/rphyrin/diary/408054)"
